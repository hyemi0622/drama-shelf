...
3. **Authentication → Users → Add user → Create new user** 로 계정 생성. 아이디가 `hyejin` 이면 이메일 칸에 `hyejin@drama.app`, 비밀번호 입력, Auto Confirm User 체크.
   사이트에서는 아이디 `hyejin` 만 입력하면 됩니다 (`@drama.app` 은 자동으로 붙음).
...
- 처음 열면 로그인 화면 → 아이디와 비밀번호 입력. 한 번 로그인하면 그 브라우저에서는 계속 유지됩니다.
